    ...
    public :
        ...
        void display_doctor_interface() {
            ...
            while (menu) {
                ...
                std::cout << "\n   Welcome " << doc.get_name() << " " << doc.get_surname() << "!\n\n\n" ;
                ...
                if (this->user == "patient") {
                    ...
                    switch (c) {
                        ...
                        case '1' : {
                            ...
                            std::vector<Radiography> results = search_by_date( day , month , year, pat.get_listRadiographies() ) ;
                            if (results.empty()){
                                std::cout << "   ╔══════════════════════════════════════════╗\n" ;
                                std::cout << "   ║   No radiographies found for this date   ║\n" ;
                                std::cout << "   ╚══════════════════════════════════════════╝\n\n" ;
                                break ;
                            } else {
                                for (size_t i=0 ; i<results.size() ; i++) {
                                    std::cout << "   ║" ;
                                    std::cout << results[i].get_id() << add_tabulation(results[i].get_id(), 13) << "║" ;
                                    if (results[i].get_type() == xRay) {
                                        std::cout << "xRay" << add_tabulation("xRay", 13) << "║" ;
                                    } else if (results[i].get_type() == MRI) {
                                        std::cout << "MRI" << add_tabulation("MRI", 13) << "║" ;
                                    } else {
                                        std::cout << "ultrasound" << add_tabulation("ultrasound", 13) << "║" ;
                                    }
                                    if (results[i].get_state() == done) {
                                        std::cout << "done" << add_tabulation("done", 13) << "║" ;
                                    } else {
                                        std::cout << "pending" << add_tabulation("pending", 13) << "║" ;
                                    }
                                    std::cout << results[i].get_day() << add_tabulation(std::to_string(results[i].get_day()), 13) << "║" ;
                                    std::cout << results[i].get_month() << add_tabulation(std::to_string(results[i].get_month()), 13) << "║" ;
                                    std::cout << results[i].get_year() << add_tabulation(std::to_string(results[i].get_year()), 13) << "║\n" ;
